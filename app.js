...
app.post('/login', async (req, res) => {
  res.status(400).send({
    error: "Use Firebase Client SDK for login and send token to backend for verification.",
  });
});
...
app.post('/users', async (req, res) => {
  ...
  if (!userId || !email || !username) {
    return res.status(400).send({ error: 'userId, email, and username are required' });
  }
